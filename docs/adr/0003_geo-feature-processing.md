...
* Recipe is replaced (`PUT`) with `status: 'created'`
  ...
  * enqueue async job that will carry out all the following steps

  * set job status to `started` (so that we can show this as running when a user
    retrieves the status of async jobs for the scenario - this kind of job needs
    to be added to the status payload)
...
The payload (`GeoFeatureSetResponseDTO`) is similar to what is described in the
previous section: it is normalized, and we extend each node that contains a
`featureId` property with the `metadata` of the corresponding feature; this
includes the `properties` K/V record that the frontend app uses to render lists
of properties when displaying splits as configured.

### Retrieving the calculated set of features for a scenario
