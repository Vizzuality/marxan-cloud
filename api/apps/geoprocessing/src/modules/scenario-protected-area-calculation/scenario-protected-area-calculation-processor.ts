  ...
  async process(job: Job<JobInput, true>): Promise<true> {
    ...
    const query = `
      with pu as (
        ...
        pu_pa_agg as (
      select id, st_transform(st_makevalid(st_intersection(pu_the_geom, pa_the_geom)), 3410) as pa_the_geom
      from pu_pa_union
      where st_intersects(pu_the_geom
          , pa_the_geom))
          , result as (
      select id, round(st_area(ST_Union(pa_the_geom))) as protected_area
      from pu_pa_agg
      group by id)
