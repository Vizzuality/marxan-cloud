...
export class ScenarioRunResultsPieceExporter implements ExportPieceProcessor {
  private readonly logger: Logger = new Logger(
    ScenarioRunResultsPieceExporter.name,
  );
  ...
  async run(input: ExportJobInput): Promise<ExportJobOutput> {
    const blmResults: PreviousBlmResultsSelectResult[] =
    ...
    const marxanRunSelectResult: MarxanRunSelectResult[] =
      await this.geoprocessingEntityManager
        .createQueryBuilder()
    ...
    const outputScenarioSummaries: OutputSummariesSelectResult[] =
