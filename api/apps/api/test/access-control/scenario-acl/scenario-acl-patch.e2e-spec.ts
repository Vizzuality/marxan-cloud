...
test(`add every type of user to a scenario as scenario owner`, async () => {
  ...
  const ownerResponse =
    await fixtures.WhenAddingANewOwnerToTheScenarioAsOwner(scenarioId);
...
test(`adds a not allowed user role to project`, async () => {
  ...
  const incorrectRoleResponse =
    await fixtures.WhenAddingIncorrectUserRole(scenarioId);
...
test(`adds nonsensical userId`, async () => {
  ...
  const nonsenseUserIdResponse =
    await fixtures.WhenAddingNonsenseUserId(scenarioId);
...
test(`adds non-existent userId`, async () => {
  ...
  const nonExistentUserIdResponse =
    await fixtures.WhenAddingNonExistentUserId(scenarioId);
...
test(`changes user role after user is soft-deleted from the app`, async () => {
  ...
  const response =
    await fixtures.WhenChangingUserRoleForDeletedUser(scenarioId);
