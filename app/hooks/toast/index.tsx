...
const ToastContext = createContext<ToastContextProps>({
  add: (id, content, options) => {
    console.info(id, content, options);
  },
  remove: (id) => {
    console.info(id);
  },
...
export function ToastProvider({ children, placement }: ToastProviderProps) {
  const [toasts, setToasts] = useState<ToastItemProps[]>([]);

  const add = useCallback(
    (id: string, content: ToastContent, options: ToastItemOptionsProps) => {
      ...
      return setToasts([...toasts, newToast]);
    },
    [toasts, setToasts]
  );
